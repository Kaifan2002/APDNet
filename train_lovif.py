...
from datasets.lovif_dataset import LOLv2Dataset, LOLv2Dataset_Train
...
def calculate_lpips(img1, img2, lpips_metric, device):
    ...
    with torch.no_grad():
        value = lpips_metric(img1_tensor.to(device), img2_tensor.to(device))
    return value.item()

def calculate_lpips_tensor(pred, gt, lpips_metric):
...
def main(args):
    ...
    best_lovif_score, best_lovif_epoch = 0.0, 0
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        lovif_score = 10 * avg_ssim + avg_psnr - 5 * avg_lpips
        if lovif_score > best_lovif_score and is_main_process():
            best_lovif_score = float(lovif_score)
            best_lovif_epoch = epoch
            os.makedirs(model_path, exist_ok=True)
            torch.save(model_without_ddp.state_dict(), os.path.join(model_path, f"best_lovif_model.pth"))
            logger.info(f"🎯 Best LOVIF score updated to {best_lovif_score:.4f} at epoch {epoch+1}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pth_dir', type=str, default="snapshots/Luner_multi/", help='模型保存目录')
    ...
    # yaml 多尺度配置
    parser.add_argument('--yml_path', type=str, default="./configs/Lovif.yaml")
