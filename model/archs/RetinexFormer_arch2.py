...
# import cv2
import numbers
from model.archs.histoformer_arch import TransformerBlock as hist
...
class PreNorm(nn.Module):
    def __init__(self, dim, fn):
        ...
        #self.norm = DynamicTanh(normalized_shape=dim, channels_last=True, alpha_init_value=0.5)
...
class space_processing(nn.Module):
    def __init__(self,inc, outc):
        ...
        self.body = nn.Sequential(
            ...
            nn.LeakyReLU(0.01),
            nn.Conv2d(in_channels=outc, out_channels=outc, kernel_size=1, stride=1, padding=0, bias=False),
            nn.LeakyReLU(0.01),
            nn.Conv2d(in_channels=outc, out_channels=inc, kernel_size=3, stride=1, padding=1, bias=False),
            nn.LeakyReLU(0.01)
        )
    def forward(self, x):
        x = self.body(x)
...
class Cross_attention(nn.Module):
    ...
    def forward(self, x, s):
        ...
        out = out.permute(0, 2, 3, 1) #0801注释掉了这个
        return out
    ...
    def __init__(
        ...
        for _ in range(num_blocks):
            self.blocks.append(nn.ModuleList([
                ...
                LayerNorm(dim),
                Cross_attention(dim, num_heads=heads,bias=False),
...
class Denoiser(nn.Module):
    def __init__(self, in_dim=3, out_dim=3, dim=31, level=2, num_blocks=[2, 4, 4]):
        ...
        for i in range(level):
            self.encoder_layers.append(nn.ModuleList([
                ...
                hist(dim=dim_level, num_heads=dim_level // dim,  ffn_expansion_factor=2.7, bias=False, LayerNorm_type='WithBias'),
        ...
        self.bottleneck = hist(dim=dim_level, num_heads=dim_level // dim, ffn_expansion_factor=2.7, bias=False, LayerNorm_type='WithBias')
        # self.bottleneck = nn.Sequential(*[hist(dim=dim_level, num_heads=dim_level // dim, ffn_expansion_factor=2.7, bias=False, LayerNorm_type='WithBias') for _ in range(num_blocks[-1])])
        ...
        for i in range(level):
            self.decoder_layers.append(nn.ModuleList([
                ...
                hist(dim=dim_level // 2, num_heads=(dim_level // 2) // dim, ffn_expansion_factor=2.7, bias=False, LayerNorm_type='WithBias'),
            ]))
            dim_level //= 2
    ...
    def forward(self, x, illu_fea):
        ...
        fea_encoder = []
        illu_fea_list = []
        for (hist, FeaDownSample, IlluFeaDownsample) in self.encoder_layers:
            fea = hist(fea,illu_fea)  # bchw
